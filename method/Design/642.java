...
class AutocompleteSystem {
    ...
    public void add(String s,int times){
        ...
        for(char c: s.toCharArray()){
            ...
            cur = next;
            cur.cnt.put(s, cur.cnt.getOrDefault(s,0)+times);
    ...
    public List<String> input(char c) {
        ...
        for(char cc : prefix.toCharArray()){
            Trie next = cur.children.get(cc);
            if(next==null) return new ArrayList<String>();
            cur = next;
        }
        PriorityQueue<Map.Entry<String,Integer>> pq = new PriorityQueue<>(new cmp());
        for(Map.Entry<String,Integer> m : cur.cnt.entrySet()){
            pq.add(m);
