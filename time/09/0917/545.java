...
class Solution {
    public List<Integer> boundaryOfBinaryTree(TreeNode root) {
        ...
        leftRecur(root.left,lst);
        leaveRecur(root,lst);
        rightRecur(root.right,lst);
        return lst;
    }
    public void leftRecur(TreeNode root,List<Integer> lst){
        if(root==null||(root.left==null&&root.right==null))
            return ;
        lst.add(root.val);
        if(root.left==null) leftRecur(root.right,lst);
        else leftRecur(root.left,lst);
       
    }
    public void rightRecur(TreeNode root,List<Integer> lst){
        if(root==null||(root.left==null&&root.right==null))
            return ;
        
        if(root.right==null) rightRecur(root.left,lst);
        else rightRecur(root.right,lst);
        lst.add(root.val);
    }
    public void leaveRecur(TreeNode root,List<Integer> lst){
        if(root==null) return;
        if(root.left==null&&root.right==null){
            lst.add(root.val);
        }
        leaveRecur(root.left,lst);
        leaveRecur(root.right,lst);
    }
}